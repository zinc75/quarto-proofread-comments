...
The **Quarto Comments** extension adds collaboration-friendly annotations to Quarto documents. You can **insert** a margin note anywhere, or **highlight a span of existing text and attach a note to it** — as comments, to-dos, notes, or questions. Annotations render as styled margin callouts and flowing in-text marks in HTML, as [`todonotes`](https://ctan.org/pkg/todonotes) margin notes + marker-pen highlights in PDF/LaTeX, and as plain text elsewhere. They can be toggled globally and coloured per author.
...
Declaring authors and tuning the rendering via an [`extensions.quarto-comments` block](#configuration) is optional but recommended.
...
```markdown
Empty brackets INSERT a comment in the margin []{.comment author="vg" note="A remark."}.

Wrapping text HIGHLIGHTS it and attaches the note to that span:
[this passage]{.comment author="vg" note="A remark about the highlighted text."}.

An inline badge stays in the running text []{.comment author="vg" inline=true note="Quick aside."}.
```

| Attribute | Values | Description |
|-----------|--------|-------------|
| *(bracket content)* | empty / text | **empty** → inserted comment; **text** → highlight that text |
| `note` | string | the comment text shown in the margin (Markdown and `$maths$` are rendered) |
| `type` | `comment` \| `todo` \| `note` \| `question` | icon + colour family (default `comment`) |
| `author` | string | matches a key in the configuration; sets the colour and label |
| `inline` | boolean | empty bracket only: render an inline badge instead of a margin note |

- `type` and `author` are independent: a `todo` by `vg` shows the to-do icon in Vincent's colour.
...
Options live under `extensions.quarto-comments` in the front matter. Everything has a sensible default, so a minimal setup just declares your authors:
...
| `show_author` | boolean | `true` | show the author label on each comment |
| `show_list` | boolean | `false` | PDF: prepend a styled, clickable list of all comments |
| `connector` | `numbered` \| `bezier` | `numbered` | PDF in-text marker for inserted comments: clickable icon + number, or a legacy Bézier curve |
...
| `authors` | map | *(none)* | per-author `name`, with optional `color_html` / `color_latex` (see below) |

### Author colours

| Comment | Display name | Colour |
|---------|--------------|--------|
| `author="vg"`, declared in `authors:` | the entry's `name:` | manual override, else auto-assigned from a Bootstrap-5 palette |
| `author="sm"`, **not** declared | `sm` (the identifier) | auto-assigned from the palette |
| **no `author`** | *(none — label suppressed)* | **neutral grey**, regardless of `type` |

Auto-assignment hashes the name into the palette, so each author gets a distinct, stable colour with no configuration — and the **same** colour is used in HTML and PDF. To override:
...
- `show_author: false` hides author labels everywhere.
...
`wide_margins: true` (the default) widens the physical page and reserves a dedicated annotation zone at the outer edge, leaving the text block exactly where `geometry` (or the class default) puts it. The zone has a light grey background, a dashed separator at the original page boundary, and a "Comments" label.
...
- **Inserted comments** render as Quarto-style margin callouts, coloured per author, each leaving a small clickable in-text anchor. A comment written mid-sentence floats to the margin without breaking the paragraph.
...
- `show_list: true` prepends a clickable list of all comments, numbered to match the markers.
...
It demonstrates inserted comments, highlights (of text, of a formula, and of a system), inline badges, the four comment types, author colours (declared, undeclared, and none), and the comment list. Use it as a starting point for wiring the extension into your own projects.
