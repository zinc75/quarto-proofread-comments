...
#
# Requires poppler (pdftoppm) and ImageMagick 7 (magick / montage).
set -euo pipefail
...
for pdf in "$DIR"/*.pdf; do
  ...
  for ((j = 0; j < ${#cards[@]}; j += 2)); do
    ...
    if [ $((j + 1)) -lt ${#cards[@]} ]; then
      magick "${cards[j]}" "${cards[j + 1]}" -background none +smush 24 "$row"
    else
      cp "${cards[j]}" "$row"
    fi
    rows+=( "$row" )
  done
  magick "${rows[@]}" -background none -smush 24 -background white -flatten \
    "$DIR/${slug}.thumb.png"
  rm -rf "$tmp"
